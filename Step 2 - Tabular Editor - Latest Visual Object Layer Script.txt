...
/************************************************************************************************************
    ...
    1. Open Tabular Editor 3 and paste this code into the Advanced Scripting window of Tabular Editor.
    2. Define Base folder path used in Step 1 and then Run within Tabular Editor 3

************************************************************************************************************/

// User Parameters - Enter same base folder path used in Step 1
string baseFolderPath = @"C:\Report Backups"; // Base folder path
string DateToday = DateTime.Now.ToString("yyyy-MM-dd"); // Get the current date in "Year-Month-Day" format
string pbiFolderName = Path.Combine(baseFolderPath, DateToday); // Combine the base folder path with the current date to create a new folder path
...
sb_Bookmarks.Append("ReportName" + '\t' + "BookmarkName" + '\t' + "BookmarkId" + '\t' + "PageId" + '\t' + "VisualId" + '\t' + "VisualHiddenFlag" + '\t' + "ReportDate" + newline);

var sb_Pages = new System.Text.StringBuilder();
sb_Pages.Append("ReportName" + '\t' + "PageId" + '\t' + "PageName" + '\t' + "PageNumber" + '\t' + "PageWidth" + '\t' + "PageHeight" + '\t' + "PageHiddenFlag" + '\t' + "VisualCount" + '\t' + "PageBackgroundImage" + '\t' + "PageWallpaperImage" + '\t' + "PageType" + '\t' + "ReportDate" + newline);

var sb_Visuals = new System.Text.StringBuilder();
sb_Visuals.Append("ReportName" + '\t' + "PageName" + '\t' + "VisualId" + '\t' + "VisualName" + '\t' + "VisualType" + '\t' + "CustomVisualFlag" + '\t' + "VisualHiddenFlag" + '\t' + "X_Coordinate" + '\t' + "Y_Coordinate" + '\t' + "Z_Coordinate" + '\t' + "VisualWidth" + '\t' + "VisualHeight" + '\t' + "ObjectCount" + '\t' + "ShowItemsNoDataFlag" + '\t' + "SlicerType" + '\t' + "ParentGroup" + '\t' + "ReportDate" + newline);

var sb_Connections = new System.Text.StringBuilder();
sb_Connections.Append("ReportName" + '\t' + "ServerName" + '\t' + "DatabaseName" + '\t' + "ReportId" + '\t' + "ConnectionType" + '\t' + "ReportDate" + newline);
...
// Use comma instead of tab for CSV format
char delimiter = ',';
string newline = Environment.NewLine;

// Function to escape CSV special characters and enclose fields with double-quotes if necessary
string EscapeForCsv(string value) {
    ...
    // Wrap the escaped value in double quotes and return
    return "\"" + escapedValue + "\"";
}

// Define the headers for each CSV file
string customVisualsHeaders = "ReportName,Name,ReportDate" + newline;
string reportFiltersHeaders = "ReportName,DisplayName,TableName,ObjectName,ObjectType,FilterType,ReportDate" + newline;
string pageFiltersHeaders = "ReportName,PageId,PageName,DisplayName,TableName,ObjectName,ObjectType,FilterType,ReportDate" + newline;
string visualFiltersHeaders = "ReportName,PageName,VisualId,TableName,ObjectName,ObjectType,FilterType,DisplayName,ReportDate" + newline;
string visualObjectsHeaders = "ReportName,PageName,VisualId,VisualType,CustomVisualFlag,TableName,ObjectName,ObjectType,Source,DisplayName,ReportDate" + newline;
string bookmarksHeaders = "ReportName,Name,Id,PageId,VisualId,VisualHiddenFlag,ReportDate" + newline;
string pagesHeaders = "ReportName,Id,Name,Number,Width,Height,HiddenFlag,VisualCount,BackgroundImage,WallpaperImage,Type,ReportDate" + newline;
string visualsHeaders = "ReportName,PageName,Id,Name,Type,CustomVisualFlag,HiddenFlag,X,Y,Z,Width,Height,ObjectCount,ShowItemsNoDataFlag,SlicerType,ParentGroup,ReportDate" + newline;
string connectionsHeaders = "ReportName,ServerName,DatabaseName,ReportID,Type,ReportDate" + newline;
string visualInteractionsHeaders = "ReportName,PageName,SourceVisualID,TargetVisualID,TypeID,Type,ReportDate" + newline;

// Initialize StringBuilders with headers
StringBuilder sb_CustomVisuals = new StringBuilder(customVisualsHeaders);
StringBuilder sb_ReportFilters = new StringBuilder(reportFiltersHeaders);
StringBuilder sb_PageFilters = new StringBuilder(pageFiltersHeaders);
StringBuilder sb_VisualFilters = new StringBuilder(visualFiltersHeaders);
StringBuilder sb_VisualObjects = new StringBuilder(visualObjectsHeaders);
StringBuilder sb_Bookmarks = new StringBuilder(bookmarksHeaders);
StringBuilder sb_Pages = new StringBuilder(pagesHeaders);
StringBuilder sb_Visuals = new StringBuilder(visualsHeaders);
StringBuilder sb_Connections = new StringBuilder(connectionsHeaders);
StringBuilder sb_VisualInteractions = new StringBuilder(visualInteractionsHeaders);

// Add results to StringBuilders for CSV format
foreach (var x in CustomVisuals.ToList())
{
    sb_CustomVisuals.Append(EscapeForCsv(ReportName) + delimiter + EscapeForCsv(x.Name) + delimiter + ReportDate + newline);
}
foreach (var x in ReportFilters.ToList())
{
    sb_ReportFilters.Append(EscapeForCsv(ReportName) + delimiter + EscapeForCsv(x.displayName) + delimiter + EscapeForCsv(x.TableName) + delimiter + EscapeForCsv(x.ObjectName) + delimiter + EscapeForCsv(x.ObjectType) + delimiter + EscapeForCsv(x.FilterType) + delimiter + ReportDate + newline);
}
foreach (var x in PageFilters.ToList())
{
    sb_PageFilters.Append(EscapeForCsv(ReportName) + delimiter + EscapeForCsv(x.PageId) + delimiter + EscapeForCsv(x.PageName) + delimiter + EscapeForCsv(x.displayName) + delimiter + EscapeForCsv(x.TableName) + delimiter + EscapeForCsv(x.ObjectName) + delimiter + EscapeForCsv(x.ObjectType) + delimiter + EscapeForCsv(x.FilterType) + delimiter + ReportDate + newline);
}
foreach (var x in VisualFilters.ToList())
{
    sb_VisualFilters.Append(EscapeForCsv(ReportName) + delimiter + EscapeForCsv(x.PageName) + delimiter + EscapeForCsv(x.VisualId) + delimiter + EscapeForCsv(x.TableName) + delimiter + EscapeForCsv(x.ObjectName) + delimiter + EscapeForCsv(x.ObjectType) + delimiter + EscapeForCsv(x.FilterType) + delimiter + EscapeForCsv(x.displayName) + delimiter + ReportDate + newline);
}
foreach (var x in VisualObjects.ToList())
{
    sb_VisualObjects.Append(EscapeForCsv(ReportName) + delimiter + EscapeForCsv(x.PageName) + delimiter + EscapeForCsv(x.VisualId) + delimiter + EscapeForCsv(x.VisualType) + delimiter + x.CustomVisualFlag + delimiter + EscapeForCsv(x.TableName) + delimiter + EscapeForCsv(x.ObjectName) + delimiter + EscapeForCsv(x.ObjectType) + delimiter + EscapeForCsv(x.Source) + delimiter + EscapeForCsv(x.displayName) + delimiter + ReportDate + newline);
}
foreach (var x in Bookmarks.ToList())
{
    sb_Bookmarks.Append(EscapeForCsv(ReportName) + delimiter + EscapeForCsv(x.Name) + delimiter + EscapeForCsv(x.Id) + delimiter + EscapeForCsv(x.PageId) + delimiter + EscapeForCsv(x.VisualId) + delimiter + x.VisualHiddenFlag + delimiter + ReportDate + newline);
}
foreach (var x in Pages.ToList())
{
    sb_Pages.Append(EscapeForCsv(ReportName) + delimiter + EscapeForCsv(x.Id) + delimiter + EscapeForCsv(x.Name) + delimiter + x.Number + delimiter + x.Width + delimiter + x.Height + delimiter + x.HiddenFlag + delimiter + x.VisualCount + delimiter + EscapeForCsv(x.BackgroundImage) + delimiter + EscapeForCsv(x.WallpaperImage) + delimiter + EscapeForCsv(x.Type) + delimiter + ReportDate + newline);
}
foreach (var x in Visuals.ToList())
{
    sb_Visuals.Append(EscapeForCsv(ReportName) + delimiter + EscapeForCsv(x.PageName) + delimiter + EscapeForCsv(x.Id) + delimiter + EscapeForCsv(x.Name) + delimiter + EscapeForCsv(x.Type) + delimiter + x.CustomVisualFlag + delimiter + x.HiddenFlag + delimiter + x.X + delimiter + x.Y + delimiter + x.Z + delimiter + x.Width + delimiter + x.Height + delimiter + x.ObjectCount + delimiter + x.ShowItemsNoDataFlag + delimiter + EscapeForCsv(x.SlicerType) + delimiter + EscapeForCsv(x.ParentGroup) + delimiter + ReportDate + newline);
}
foreach (var x in Connections.ToList())
{
    sb_Connections.Append(EscapeForCsv(ReportName) + delimiter + EscapeForCsv(x.ServerName) + delimiter + EscapeForCsv(x.DatabaseName) + delimiter + EscapeForCsv(x.ReportID) + delimiter + EscapeForCsv(x.Type) + delimiter + ReportDate + newline);
}
foreach (var x in VisualInteractions.ToList())
{
    sb_VisualInteractions.Append(EscapeForCsv(ReportName) + delimiter + EscapeForCsv(x.PageName) + delimiter + EscapeForCsv(x.SourceVisualID) + delimiter + EscapeForCsv(x.TargetVisualID) + delimiter + x.TypeID + delimiter + EscapeForCsv(x.Type) + delimiter + ReportDate + newline);
}
...
if (saveToFile)
{
    ...
    string customVisualsFilePath = pbiFolderName + @"\CustomVisuals.csv";
    string reportFiltersFilePath = pbiFolderName + @"\ReportFilters.csv";
    string pageFiltersFilePath = pbiFolderName + @"\PageFilters.csv";
    string visualFiltersFilePath = pbiFolderName + @"\VisualFilters.csv";
    string visualObjectsFilePath = pbiFolderName + @"\VisualObjects.csv";
    string bookmarksFilePath = pbiFolderName + @"\Bookmarks.csv";
    string pagesFilePath = pbiFolderName + @"\Pages.csv";
    string visualsFilePath = pbiFolderName + @"\Visuals.csv";
    string connectionsFilePath = pbiFolderName + @"\Connections.csv";
    string visualInteractionsFilePath = pbiFolderName + @"\VisualInteractions.csv";

    // Create a dictionary to map each StringBuilder to its corresponding file path
    Dictionary<StringBuilder, string> stringBuilderToFileMap = new Dictionary<StringBuilder, string>
    {
        { sb_CustomVisuals, customVisualsFilePath },
        { sb_ReportFilters, reportFiltersFilePath },
        { sb_PageFilters, pageFiltersFilePath },
        { sb_VisualFilters, visualFiltersFilePath },
        { sb_VisualObjects, visualObjectsFilePath },
        { sb_Bookmarks, bookmarksFilePath },
        { sb_Pages, pagesFilePath },
        { sb_Visuals, visualsFilePath },
        { sb_Connections, connectionsFilePath },
        { sb_VisualInteractions, visualInteractionsFilePath }
    };

    // Loop through each StringBuilder and its corresponding file path
    foreach (var kvp in stringBuilderToFileMap)
    {
        StringBuilder sb = kvp.Key;
        ...
        else
        {
            ...
            File.AppendAllText(filePath, sb.ToString().Substring(sb.ToString().IndexOf(Environment.NewLine) + Environment.NewLine.Length));
        }
    }
}
...
{
    // If you have a custom Output method for StringBuilders, ensure it outputs CSV format
    Console.WriteLine(sb_CustomVisuals.ToString());
    Console.WriteLine(sb_ReportFilters.ToString());
    Console.WriteLine(sb_PageFilters.ToString());
    Console.WriteLine(sb_VisualFilters.ToString());
    Console.WriteLine(sb_VisualObjects.ToString());
    Console.WriteLine(sb_Visuals.ToString());
    Console.WriteLine(sb_Bookmarks.ToString());
    Console.WriteLine(sb_Pages.ToString());
    Console.WriteLine(sb_Connections.ToString());
    Console.WriteLine(sb_VisualInteractions.ToString());
}
...
if (createPersp)
{
    ...
    if (saveToFile)
    {
        System.IO.File.AppendAllText(pbiFolderName+@"\"+savePrefix+"_UnusedObjects.txt", sb_Unused.ToString());
    }
    else
    {
        sb_Unused.Output();
